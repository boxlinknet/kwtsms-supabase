...
<body>
  <div id="login-screen">
    <div id="login-box">
      ...
      <div class="form-group" id="project-url-group">
        <label>Supabase Project URL</label>
        <input type="url" id="login-project-url" placeholder="https://xxxx.supabase.co">
      </div>
  ...
  <script>
    ...
    // API base URL: auto-detect from ?project= param, sessionStorage, or prompt
    var API_BASE = '';
    (function() {
      if (window.location.pathname.indexOf('/functions/v1/sms-dashboard') === 0) {
        API_BASE = '/functions/v1/sms-dashboard/api/';
      } else {
        // Check ?project= URL parameter (set by Edge Function redirect)
        var params = new URLSearchParams(window.location.search);
        var projectUrl = params.get('project');
        if (projectUrl) {
          projectUrl = projectUrl.replace(/\/+$/, '');
          sessionStorage.setItem('supabase_url', projectUrl);
          API_BASE = projectUrl + '/functions/v1/sms-dashboard/api/';
        } else {
    ...
    function ensureApiBase() {
      ...
      // Read from login form field
      var urlInput = document.getElementById('login-project-url');
      if (urlInput) {
    ...
    async function doLogin() {
      ...
      if (!ensureApiBase()) {
        document.getElementById('login-error').textContent = 'Project URL is required';
        return;
      }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      // Hide project URL field if served from same Supabase project
      if (API_BASE) {
        var urlGroup = document.getElementById('project-url-group');
        if (urlGroup) urlGroup.style.display = 'none';
      } else {
        // Pre-fill from sessionStorage if returning
        var saved = sessionStorage.getItem('supabase_url');
        if (saved) document.getElementById('login-project-url').value = saved;
      }
